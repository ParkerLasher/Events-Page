// Function to add a new entry to the Financial Aid Workshop sign-up table
function addName() {
    var name = document.getElementById('name').value;
    var email = document.getElementById('email').value;
    var term = document.getElementById('term').value;

    // Validation to ensure that name, email, and term are provided
    if (!name || !email || !term) {
        alert('Please enter name, email, and term.');
        return;
    }
    ...
    newRow.insertCell(3).innerHTML = term;
...
function saveTableData() {
    ...
    localStorage.setItem('financialAidData', JSON.stringify(rowsData));
}

// Function to load table data from localStorage
function loadTableData() {
    var savedData = localStorage.getItem('financialAidData');
    
    if (savedData) {
        var tableData = JSON.parse(savedData);
        var table = document.getElementById('signupTable');
        // Ensure the header is correct
        table.innerHTML = "<tr><th>Name</th><th>Email</th><th>Term</th><th>Signup Time</th><th>Delete</th></tr>";
