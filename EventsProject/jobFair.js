// Function to add a new entry to the Job Fair sign-up table
function addName() {
    var name = document.getElementById('name').value;
    var email = document.getElementById('email').value;
    var careerInterest = document.getElementById('careerInterest').value;

    // Validation to ensure that name, email, and career interest are provided
    if (!name || !email || !careerInterest) {
        alert('Please enter name, email, and career interest.');
        return;
    }
    ...
    newRow.insertCell(3).innerHTML = careerInterest;
...
function saveTableData() {
    ...
    localStorage.setItem('jobFairData', JSON.stringify(rowsData));
}

// Function to load table data from localStorage
function loadTableData() {
    var savedData = localStorage.getItem('jobFairData');
    
    if (savedData) {
        var tableData = JSON.parse(savedData);
        var table = document.getElementById('signupTable');
        // Ensure the header is correct
        table.innerHTML = "<tr><th>Name</th><th>Email</th><th>Signup Time</th><th>Career Interest</th><th>Delete</th></tr>";
